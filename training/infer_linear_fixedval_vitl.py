...
def evaluate(backbone, head, loader, use_amp: bool, device: torch.device):
    ...
    for pixel_values, y, subclasses in it:
        ...
        # per-image probability for class 1 ("fake")
...
def main():
    ...
    parser.add_argument("--ckpt", type=str, default="checkpoints/dinov3_linear_head_best_vitl.pt")
    parser.add_argument("--batch_size", type=int, default=64)
    parser.add_argument("--num_workers", type=int, default=16)
    parser.add_argument("--amp", action="store_true")
    parser.add_argument("--csv", type=str, default="eval_results.csv", help="保存数据集汇总结果的CSV路径（默认：eval_results.csv）")
    parser.add_argument("--pred_csv", type=str, default="pred_results.csv", help="保存每张图片预测结果的CSV路径（默认：pred_results.csv）")
    ...
    results = []
    pred_rows = []  # accumulate (filename, prob)
    ...
    for root in args.data_roots:
        ...
        for sub in subdirs:
            ...
            # 输出并收集每张图片的预测概率（class=1 为 fake 概率）
            for fname, prob in per_image:
                # print(f"    图片名称：{fname}  预测概率：{prob:.6f}")
                acc_file.write(fname + '\t' + str(prob) + '\n')
                pred_rows.append((fname, prob))
    ...
    if pred_rows:
        with open(args.pred_csv, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["图片名称", "预测概率(假)"])  # 概率为类1（fake）
            for fname, prob in pred_rows:
                writer.writerow([fname, f"{prob:.6f}"])
        print(f"[Saved] Per-image predictions CSV: {args.pred_csv}")


if __name__ == "__main__":
    main()

# 请你修改并简化上述代码，使得logit结果能保存到csv文件中
